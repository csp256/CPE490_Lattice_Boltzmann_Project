...
__global__ void tiledKernel (float *fIn, float *fOut, char *bbRegion, int iteration) {
	...
	for (int step = 0; step < stepsPerKernel;) {
		...
		if (onGrid && activeThread && isObstacle!=1) {
		...
		__syncthreads();
		if (x == in && 0 < y && y < ly-1 && isObstacle!=1) {
			ux = 4.0f * uMax / ((ly-2.0f)*(ly-2.0f)) * ((y-0.5f)*(ly-2.0f) - (y-0.5f)*(y-0.5f));
			uy = 0.0f;
			rho = 1.0f / (1.0f - ux) * ( (fR[0]+fR[2]+fR[4]) + 2*(fR[3]+fR[6]+fR[7]) )/rho ;
			fR[1] = fR[3] + 2.0f/3.0f * rho * ux; 
			fR[5] = fR[7] + 0.5f*(fR[4]-fR[2]) + 0.5f*rho*uy + 1.0f/6.0f*rho*ux;
			fR[8] = fR[6] + 0.5f*(fR[2]-fR[4]) - 0.5f*rho*uy + 1.0f/6.0f*rho*ux;
		}
			// Outlet: constant pressure
		__syncthreads();
		if (x == out && 0 < y && y < ly-1 && isObstacle!=1) { 
			rho = 1.0f;
			ux = -1.0f + 1.0f / rho * ( (fR[0]+fR[2]+fR[4]) + 2*(fR[1]+fR[5]+fR[8]) ) ;
			uy = 0.0f;
			fR[3] = fR[1] - 2.0f/3.0f * rho * ux;
			fR[7] = fR[5] + 0.5f*(fR[2]-fR[4]) - 0.5f*rho*uy - 1.0f/6.0f*rho*ux;
			fR[6] = fR[8] + 0.5f*(fR[4]-fR[2]) + 0.5f*rho*uy - 1.0f/6.0f*rho*ux;
		}

		// Bounce-back or collide
		__syncthreads();
		if (onGrid && activeThread) {
			if (isObstacle == 1) {
				#pragma unroll
				for (int k = 0; k < 9; ++k) {
					fS[dirS(k)] = fR[opp[k]]; // need to change opp for this to be constant index
				}
			} else {
				float cu;
				#pragma unroll
				for (int k = 0; k < 9; ++k) {
					...
					fS[dirS(k)] = fR[k] - omega * (fR[k] - temp);
				}
			}
		}
...
*/		
				fR[k] = fS[xSource + ySource*bdx + k*bdx*bdy];
			}
		}
	}
